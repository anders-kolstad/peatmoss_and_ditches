...
I thought I could also remove those that are new in the fall, but turn out these could be labeles new in spring, and then that label is kept though that season (and reset again in the next season).
...
I need to remove those that are new in fall and that don't have any values labels new in spring (i.e. that they are truly new that fall, and that the label is not simply carried from the spring record).
...
<!--# dat_season |> filter(is.na(growth_cm)) |> View() -->
